...
public class UsersController {
	...
	public ResponseEntity<UserResponseModel> getUser(
		...
		return ResponseEntity.status(HttpStatus.FOUND)
				.body(new ModelMapper().map(user, UserResponseModel.class));
	}
	...
	public ResponseEntity<CreateUserResponseModel> createUser(
		...
		UserDto userResponseDto = usersService
				.createUser(modelMapper.map(userDetails, UserDto.class));

		CreateUserResponseModel createUserResponseModel = modelMapper
				.map(userResponseDto, CreateUserResponseModel.class);

		return ResponseEntity.status(HttpStatus.CREATED)
				.body(createUserResponseModel);
	}
	...
	public ResponseEntity<UpdateUserResponseModel> updateUser(
			@Valid @RequestBody final CreateUserRequestModel userDetails) {
		ModelMapper modelMapper = new ModelMapper();
		final UpdateUserResponseModel returnValue = modelMapper.map(userDetails,
